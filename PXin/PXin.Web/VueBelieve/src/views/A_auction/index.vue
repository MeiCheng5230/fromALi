<template>
    <div class="Aauction">
        ...
        <div class="thisMonth noRecord" v-show="!isdefault" v-else :style="{backgroundImage:'url('+require('@/assets/images/paimai_unrecorded_bg.png')+')'}">
        ...
        <div class="record">
             <div class="top">
                 <button class="bntRefresh" :disabled="disable" :class="{ codeGeting:isGeting }" tag="button" @click="saveRefreshNum" >{{getCode}}</button>
            </div>
            <div class="top">
                <div class="lft">拍卖记录</div>
                <router-link to="/charts" tag="div" class="rgt">查看更多<img src="@/assets/images/ic_enter@2x.png" alt=""></router-link>
            </div>
            <div class="tit">
        ...
        <div class="fixedbx">
                ...
                <div> 
                    ...
                    <van-count-down :time="time">
                        <template v-slot="timeData">
                            <span class="item"><span>{{ String(timeData.hours).length == 1 ? '0'+ timeData.hours : timeData.hours  }}</span>小时</span>
            ...
            <div class="btnbx">  
                <div class="rmk" v-if="parseInt(time/1000/60/60/24) <= 0 && time>0" :style="{backgroundImage:'url('+require('@/assets/images/paimai_hint_bg.png')+')'}">今天出局仅退还一半P点哦，快来加价</div>
...
import { GetThisMonthData, SetAgreeAgreement, GetAuctionAddprice,GetAuctionRanking } from '@/api/getData';
...
export default {
    ...
    created(){
      ...
      this.draw = '/App/Luckdraw/index.html'+location.search;
    },
    methods:{
         ...
         agreeok(){
           ...
           //VueCookies.set(nodeid+"_auction",1);
           this.popFlag = false;
           this.$router.push({name:'bidsauction'});
         },
         myauction(){
             if (this.time < 0) {
                 Dialog.alert({
