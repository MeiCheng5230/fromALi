...
<template>
  <div class="test">
    <table></table>
    <div class="info">
      <input v-model="mobileno" type="text" readonly />
    ...
    <!-- 按钮 -->
    <div class="btn">
      <button :class="code && 'active'" @click="Next()">{{$t('m.next')}}</button>
    </div>
  </div>
</template>

<script>
import { SendSms, CheckVerificationCode } from "@/api/sysRequest.js";
export default {
  data() {
    return {
      mobileno: "", //手机号
  ...
  methods: {
    ...
    Next: function() {
      CheckVerificationCode(
        { mobileno: this.mobileno, code: this.code },
        data => {
          if (data.result < 0) {
          ...
          this.$router.push({
            name: "ChangePhone",
            query: { oldMobileNo: this.mobileno, oldCode: this.code }
          });
        }
      );
    }
