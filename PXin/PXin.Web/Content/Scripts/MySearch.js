$.extend({
    myMethod: function (id,tagData,name, num, def) {
		$(id).html("");
		...
		if (def) {
			def.forEach(item => {
				...
				var id = item.id;
				if(id!=undefined){
					$(".AD").append('<a href="javascript:;" class="label"><span lay-value="64">'+item.name+'</span><input type="hidden" name="name" id="'+item.name+'" value="'+item.id+'"/><i class="layui-icon close">x</i></a>')
					$(".AD").parent().show();
	    ...
	    $(".fileId").on("click","dl dd",function(){
			...
			var addnum = $("#demo .layui-form-item .AD a").length;
			if(num) {
				if (addnum < num) {
		...
		$(".AD").on("click",".close",function(){
			if(num) {
				$(".layui-form-selected").show();
				// $(".layui-form-item .AD").hide();
			}
