...
namespace Wt.DataAccess
{
    public class OracleHelper
    {
        //数据库连接字符串  
        private readonly static string connstr = ConfigurationManager.ConnectionStrings["PXinDB"].ConnectionString;
        ...
        internal static int ExecuteNonQuery(string cmdText, params OracleParameter[] commandParameters)
        {
            ...
            {
                PXin.Common.Log.ExceptInfo(ex.ToString());
            }
            finally
            {
                command.Dispose();
        ...
        /// <param name="tabName">返回的表名，不可为空</param>
        /// <returns>数据表</returns>
        internal static DataTable GetDataTable(string cmdText, string tabName)
        {
